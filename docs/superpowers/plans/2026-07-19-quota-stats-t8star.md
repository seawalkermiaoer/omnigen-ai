...
**先处理一个既有的测试脆弱性。** Task 7 实测发现：23 个前端测试文件并行跑时，3 个**不相关**的既有测试（`UsersPage`、`R2VPage`、`HistoryPage`）会因 CPU 争用在 `userEvent` 交互上超时；单独跑各自都通过，`npx vitest run --no-file-parallelism` 下 157/157 全过。这不是本次改动引入的，但它会让最终回归给出假失败。先查清是调 `testTimeout` 还是限制并行度更合适，做掉再跑回归——否则你无法区分「真的坏了」和「机器忙」。
